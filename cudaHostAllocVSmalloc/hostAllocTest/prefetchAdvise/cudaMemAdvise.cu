...
int main () {
    ...
    for(int j = 0; j < NUMBER_OF_TESTS; j++) {
        ...
        cudaMemAdvise((void *)(h_input), numberOfElements * sizeof(int), cudaMemAdviseSetAccessedBy, device);
        cudaMemAdvise((void *)(h_other_input), numberOfElements * sizeof(int), cudaMemAdviseSetAccessedBy, device);

        timer_advise.end();
        advise_sum += timer_advise.elapsed();
    ...
    printf("Average advise time for size %ldGB is %f\n", DATA_SIZE, advise_sum / NUMBER_OF_TESTS);
