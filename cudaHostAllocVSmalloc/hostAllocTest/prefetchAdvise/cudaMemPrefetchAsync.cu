...
int main () {
    ...
    for(int j = 0; j < NUMBER_OF_TESTS; j++) {
        ...
        for (int i = 0; i < DATA_SIZE; i += BATCH_DATA_SIZE) {
            ...
            timer_prefetch.start();

            int device;
            cudaGetDevice(&device);


            cudaMemPrefetchAsync((void *)(h_input + offset), numberOfBatchElements * sizeof(int), device, stream1);
            cudaMemPrefetchAsync((void *)(h_other_input + offset), numberOfBatchElements * sizeof(int), device, stream2);

            timer_prefetch.end();
            prefetch_sum += timer_prefetch.elapsed();

            timer_kernel.start();

            int blockSize = BLOCK_SIZE;
            int numBlocks = MAX_THREAD / blockSize;
            complexKernel<<<numBlocks, blockSize, 0, stream1>>>(h_input + offset, numberOfBatchElements);
            complexKernel<<<numBlocks, blockSize, 0, stream2>>>(h_other_input + offset, numberOfBatchElements);
            timer_kernel.end();
    ...
    printf("Average prefetch time for size %ldGB is %f\n", DATA_SIZE, prefetch_sum / NUMBER_OF_TESTS);
