...
#define DATA_SIZE 16L
#define BATCH_DATA_SIZE 4L
...
int main () {
    ...
    for(int j = 0; j < NUMBER_OF_TESTS; j++) {
        ...
        int *h_input, *h_other_input, *h_second_other_input, *h_third_other_input;
        bool *h_checks, *h_other_checks, *h_second_other_checks, *h_third_other_checks;

        unsigned long long int numberOfElements = ((DATA_SIZE / 4) * (1L << 30)) / sizeof(int);
        unsigned long long int numberOfBatchElements = ((BATCH_DATA_SIZE / 4) * (1L << 30) / sizeof(int));
        ...
        cudaHostAlloc((void **) &h_second_other_input, numberOfElements * sizeof(int), cudaHostAllocDefault);
        cudaHostAlloc((void **) &h_third_other_input, numberOfElements * sizeof(int), cudaHostAllocDefault);

        cudaHostAlloc((void **) &h_checks, (MAX_THREAD / 4) * sizeof(bool), cudaHostAllocDefault);
        cudaHostAlloc((void **) &h_other_checks, (MAX_THREAD / 4) * sizeof(bool), cudaHostAllocDefault);
        cudaHostAlloc((void **) &h_second_other_checks, (MAX_THREAD / 4) * sizeof(bool), cudaHostAllocDefault);
        cudaHostAlloc((void **) &h_third_other_checks, (MAX_THREAD / 4) * sizeof(bool), cudaHostAllocDefault);
        

        for (unsigned long long int i = 0; i < numberOfElements; i++) {
            h_input[i] =  (int) (i % (1 << 29));
            h_other_input[i] = (int) ((i + numberOfElements) % (1 << 29));
            h_second_other_input[i] = (int) ((i + numberOfElements * 2) % (1 << 29));
            h_third_other_input[i] = (int) ((i + numberOfElements * 3) % (1 << 29));
        }
        ...
        int *d_input, *d_other_input, *d_second_other_input, *d_third_other_input;
        bool *d_checks, *d_other_checks, *d_second_other_checks, *d_third_other_checks;

        cudaMalloc((void **) &d_input, numberOfBatchElements * sizeof(int));
        cudaMalloc((void **) &d_other_input, numberOfBatchElements * sizeof(int));
        cudaMalloc((void **) &d_second_other_input, numberOfBatchElements * sizeof(int));
        cudaMalloc((void **) &d_third_other_input, numberOfBatchElements * sizeof(int));

        cudaMalloc((void **) &d_checks, (MAX_THREAD / 4) * sizeof(bool));
        cudaMalloc((void **) &d_other_checks, (MAX_THREAD / 4) * sizeof(bool));
        cudaMalloc((void **) &d_second_other_checks, (MAX_THREAD / 4) * sizeof(bool));
        cudaMalloc((void **) &d_third_other_checks, (MAX_THREAD / 4) * sizeof(bool));
        ...
        for (int i = 0; i < DATA_SIZE; i += BATCH_DATA_SIZE) {
            ...
            unsigned long long int offset = (i / 4) * (1L << 30) / sizeof(int);
            ...
            cudaMemcpyAsync((void *) d_second_other_input, (void *) (h_second_other_input + offset), numberOfBatchElements * sizeof(int), cudaMemcpyHostToDevice, stream3);
            cudaMemcpyAsync((void *) d_third_other_input, (void *) (h_third_other_input + offset), numberOfBatchElements * sizeof(int), cudaMemcpyHostToDevice, stream4);
            ...
            mediumKernel<<<numBlocks / 4, blockSize, 0, stream1>>>(d_input, numberOfBatchElements, d_checks);
            mediumKernel<<<numBlocks / 4, blockSize, 0, stream2>>>(d_other_input, numberOfBatchElements, d_other_checks);
            mediumKernel<<<numBlocks / 4, blockSize, 0, stream3>>>(d_second_other_input, numberOfBatchElements, d_second_other_checks);
            mediumKernel<<<numBlocks / 4, blockSize, 0, stream4>>>(d_third_other_input, numberOfBatchElements, d_third_other_checks);
            ...
            cudaMemcpyAsync((void *) (h_second_other_input + offset), (void *) d_second_other_input, numberOfBatchElements * sizeof(int), cudaMemcpyDeviceToHost, stream3);
            cudaMemcpyAsync((void *) (h_third_other_input + offset), (void *) d_third_other_input, numberOfBatchElements * sizeof(int), cudaMemcpyDeviceToHost, stream4);

            cudaMemcpyAsync((void *) (h_checks), (void *) d_checks, (MAX_THREAD / 4) * sizeof(bool), cudaMemcpyDeviceToHost, stream1);
            cudaMemcpyAsync((void *) (h_other_checks), (void *) d_other_checks, (MAX_THREAD / 4) * sizeof(bool), cudaMemcpyDeviceToHost, stream2);
            cudaMemcpyAsync((void *) (h_second_other_checks), (void *) d_second_other_checks, (MAX_THREAD / 4) * sizeof(bool), cudaMemcpyDeviceToHost, stream3);
            cudaMemcpyAsync((void *) (h_third_other_checks), (void *) d_third_other_checks, (MAX_THREAD / 4) * sizeof(bool), cudaMemcpyDeviceToHost, stream4);
        ...
        cudaFreeHost(h_second_other_input);
        cudaFreeHost(h_third_other_input);

        cudaFreeHost(h_checks);
        cudaFreeHost(h_other_checks);
        cudaFreeHost(h_second_other_checks);
        cudaFreeHost(h_third_other_checks);


        cudaFree(d_input);
        cudaFree(d_other_input);
        cudaFree(d_second_other_input);
        cudaFree(d_third_other_input);

        cudaFree(d_checks);
        cudaFree(d_other_checks);
        cudaFree(d_second_other_checks);
        cudaFree(d_third_other_checks);
