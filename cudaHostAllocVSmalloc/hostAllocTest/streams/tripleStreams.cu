...
int main () {
    ...
    for(int j = 0; j < NUMBER_OF_TESTS; j++) {
        ...
        int *h_input, *h_other_input, *h_second_other_input;
        bool *h_checks, *h_other_checks, *h_second_other_checks;

        unsigned long long int numberOfElements = ((DATA_SIZE / 3) * (1L << 30)) / sizeof(int);
        unsigned long long int numberOfBatchElements = ((BATCH_DATA_SIZE / 3) * (1L << 30) / sizeof(int));
        ...
        cudaHostAlloc((void **) &h_checks, (MAX_THREAD / 3) * sizeof(bool), cudaHostAllocDefault);
        cudaHostAlloc((void **) &h_other_checks, (MAX_THREAD / 3) * sizeof(bool), cudaHostAllocDefault);
        cudaHostAlloc((void **) &h_second_other_checks, (MAX_THREAD / 3) * sizeof(bool), cudaHostAllocDefault);
        ...
        int *d_input, *d_other_input, *d_second_other_input;
        bool *d_checks, *d_other_checks, *d_second_other_checks;
        ...
        cudaMalloc((void **) &d_checks, (MAX_THREAD / 3) * sizeof(bool));
        cudaMalloc((void **) &d_other_checks, (MAX_THREAD / 3) * sizeof(bool));
        cudaMalloc((void **) &d_second_other_checks, (MAX_THREAD / 3) * sizeof(bool));
        ...
        for (int i = 0; i < DATA_SIZE; i += BATCH_DATA_SIZE) {
            ...
            unsigned long long int offset = (i / 3) * (1L << 30) / sizeof(int);
            ...
            mediumKernel<<<numBlocks / 3, blockSize, 0, stream1>>>(d_input, numberOfBatchElements, d_checks);
            mediumKernel<<<numBlocks / 3, blockSize, 0, stream2>>>(d_other_input, numberOfBatchElements, d_other_checks);
            mediumKernel<<<numBlocks / 3, blockSize, 0, stream3>>>(d_second_other_input, numberOfBatchElements, d_second_other_checks);
            ...
            cudaMemcpyAsync((void *) (h_checks), (void *) d_checks, (MAX_THREAD / 3) * sizeof(bool), cudaMemcpyDeviceToHost, stream1);
            cudaMemcpyAsync((void *) (h_other_checks), (void *) d_other_checks, (MAX_THREAD / 3) * sizeof(bool), cudaMemcpyDeviceToHost, stream2);
            cudaMemcpyAsync((void *) (h_second_other_checks), (void *) d_second_other_checks, (MAX_THREAD / 3) * sizeof(bool), cudaMemcpyDeviceToHost, stream3);
