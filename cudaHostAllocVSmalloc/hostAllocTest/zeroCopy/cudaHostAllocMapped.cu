...
int main () {
    ...
    for(int j = 0; j < NUMBER_OF_TESTS; j++) {
        ...
        cudaHostAlloc((void **) &h_input, numberOfElements * sizeof(int), cudaHostAllocMapped);
        cudaHostGetDevicePointer(&d_map, h_input, 0);
        ...
        kernel<<<numBlocks, blockSize>>>(d_map, numberOfElements);
