...
__global__ void kernel(int* input, int* output, unsigned long long int numberOfElements) {
    ...
    int cond;
    int delay_count = 0;
    int max_delay = numberOfWorks / 10;
    for (unsigned long long int i = 0; i < numberOfWorks;) {
        unsigned long long int index = idx * numberOfWorks + i;
        if (not_delayed)
            cond = ((index % 3 != 0) && (i < numberOfWorks / 3)) || ((index % 3 != 1) && (i >= numberOfWorks / 3) && (i < (numberOfWorks / 3) * 2)) || ((index % 3 != 2) && (i >= (numberOfWorks / 3) * 2));
        unsigned int activeMask = __activemask();
        int cond_for_all = (__popc(__ballot_sync(activeMask, cond)) >= THRESH);
        if (delay_count >= max_delay)
            not_delayed = 1;
        else
            not_delayed = (cond_for_all == cond);
        if (not_delayed) {
            int dest = 0;
            int src = input[index];
            int j;
...
int main () {
    ...
    for(int j = 0; j < NUMBER_OF_TESTS; j++) {
        ...
        timer_kernel.end();
        float time = timer_kernel.elapsed();
        kernel_sum += time;
